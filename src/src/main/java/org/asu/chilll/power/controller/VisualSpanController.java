...
import org.asu.chilll.power.service.VisualSpanService;
...
@Controller
public class VisualSpanController {
	
	@Autowired
	private GameProgressService progressService;
	
	@Autowired
	private VisualSpanService gameService;
	
	@Autowired
	private REDCapSyncDataService redcapService;
	
	@RequestMapping(value = "/api/visualspan/syncdata", method = RequestMethod.POST)
	public @ResponseBody SyncDataResult initSyncDataRecord(@RequestBody String formData) {
		try {
			...
			return redcapService.initSyncDataRecords(student.getStudentId(), student.getGrade(), GameIdType.Visual_Span.toString());
		}catch(Exception e) {
	...
	@RequestMapping(value = "/api/visualspan/progress", method = RequestMethod.GET)
	public @ResponseBody GameProgressDataView fetchGameProgress(@RequestParam("studentId") String studentId, @RequestParam("grade") String grade) {
		try {
			GameProgressDataView progress = progressService.fetchGameProgress(studentId, grade, GameIdType.Visual_Span.toString());
			return progress;
		}catch(Exception e) {
			System.out.println("========================================== POWER PROGRAM ERROR ==========================================");
			e.printStackTrace(System.out);
	...
	@RequestMapping(value = "/api/visualspan/progress", method = RequestMethod.POST)
	public @ResponseBody String updateGameProgress(@RequestBody String formData) {
		try {
			...
			progress.setGameId(GameIdType.Visual_Span.toString());
	...
	@RequestMapping(value = "/api/visualspan", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
	...
	@RequestMapping(value = "/api/visualspan/gameover", method = RequestMethod.POST)
	public @ResponseBody GameProgressDataView gameOver(@RequestBody String formData) {
		try {
			GameProgressDataView progress = new Gson().fromJson(formData, GameProgressDataView.class);
			return progressService.updateGameProgressWhenGameOver(progress.getChildId(), progress.getGrade(), GameIdType.Visual_Span.toString(), null);
		}catch(Exception e) {
			System.out.println("========================================== POWER PROGRAM ERROR ==========================================");
			e.printStackTrace(System.out);
			return null;
