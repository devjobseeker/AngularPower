...
import org.asu.chilll.power.service.NumberUpdateAuditoryService;
...
@Controller
public class NumberUpdateAuditoryController {
	@Autowired
	private GameProgressService progressService;
	
	@Autowired
	private NumberUpdateAuditoryService gameService;
	...
	@RequestMapping(value = "/api/numberupdateauditory/syncdata", method = RequestMethod.POST)
	public @ResponseBody SyncDataResult initSyncDataRecord(@RequestBody String formData) {
		try {
			...
			return redcapService.initSyncDataRecords(student.getStudentId(), student.getGrade(), GameIdType.Number_Update_Auditory.toString());
		}catch(Exception e) {
	...
	@RequestMapping(value = "/api/numberupdateauditory/progress", method = RequestMethod.GET)
	public @ResponseBody GameProgressDataView fetchGameProgress(@RequestParam("studentId") String studentId, @RequestParam("grade") String grade) {
		try {
			GameProgressDataView progress = progressService.fetchGameProgress(studentId, grade, GameIdType.Number_Update_Auditory.toString());
			return progress;
		}catch(Exception e) {
			System.out.println("========================================== POWER PROGRAM ERROR ==========================================");
			e.printStackTrace(System.out);
	...
	@RequestMapping(value = "/api/numberupdateauditory/progress", method = RequestMethod.POST)
	public @ResponseBody String updateGameProgress(@RequestBody String formData) {
		try {
			...
			progress.setGameId(GameIdType.Number_Update_Auditory.toString());
	...
	@RequestMapping(value = "/api/numberupdateauditory", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
	...
	@RequestMapping(value = "/api/numberupdateauditory/gameover", method = RequestMethod.POST)
	public @ResponseBody GameProgressDataView gameOver(@RequestBody String formData) {
		try{
			GameProgressDataView progress = new Gson().fromJson(formData, GameProgressDataView.class);
			return progressService.updateGameProgressWhenGameOver(progress.getChildId(), progress.getGrade(), GameIdType.Number_Update_Auditory.toString(), null);
		}catch(Exception e) {
			System.out.println("========================================== POWER PROGRAM ERROR ==========================================");
			e.printStackTrace(System.out);
			return null;
	...
	@RequestMapping(value = "/api/numberupdateauditory/audio", method = RequestMethod.POST)
	public @ResponseBody String sendAudioData(MultipartHttpServletRequest request, @RequestParam("childId") String childId,
		...
		try {
			...
			while(itr.hasNext()) {
				...
				dv.setGrade(grade);
				dv.setGameId(GameIdType.Number_Update_Auditory.toString());
				dv.setFileLength(file.getSize());
				dv.setFileContent(file.getBytes());
				dv.setFileType(file.getContentType());
				dv.setFileName(GameIdType.Number_Update_Auditory.toString() + "_" + file.getOriginalFilename());
				dv.setFolderName(FileFolderName.Number_Update_Auditory.toString());
				dvs.add(dv);
