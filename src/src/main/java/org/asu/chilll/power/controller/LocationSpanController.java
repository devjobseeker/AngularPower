...
import org.asu.chilll.power.service.LocationSpanService;
...
@Controller
public class LocationSpanController {
	
	@Autowired
	private GameProgressService progressService;
	
	@Autowired
	private LocationSpanService gameService;
	
	@Autowired
	private REDCapSyncDataService redcapService;
	
	@RequestMapping(value = "/api/locationspan/syncdata", method = RequestMethod.POST)
	public @ResponseBody SyncDataResult initSyncDataRecord(@RequestBody String formData) {
		try {
			...
			return redcapService.initSyncDataRecords(student.getStudentId(), student.getGrade(), GameIdType.Location_Span.toString());
		}catch(Exception e) {
	...
	@RequestMapping(value = "/api/locationspan/progress", method = RequestMethod.GET)
	public @ResponseBody GameProgressDataView fetchGameProgress(@RequestParam("studentId") String studentId, @RequestParam("grade") String grade) {
		try {
			GameProgressDataView progress = progressService.fetchGameProgress(studentId, grade, GameIdType.Location_Span.toString());
			return progress;
		}catch(Exception e) {
			System.out.println("========================================== POWER PROGRAM ERROR ==========================================");
			e.printStackTrace(System.out);
	...
	@RequestMapping(value = "/api/locationspan/progress", method = RequestMethod.POST)
	public @ResponseBody String updateGameProgress(@RequestBody String formData) {
		try {
			...
			progress.setGameId(GameIdType.Location_Span.toString());
	...
	@RequestMapping(value = "/api/locationspan", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
	...
	@RequestMapping(value = "/api/locationspan/gameover", method = RequestMethod.POST)
	public @ResponseBody GameProgressDataView gameOver(@RequestBody String formData) {
		try {
			GameProgressDataView progress = new Gson().fromJson(formData, GameProgressDataView.class);
			return progressService.updateGameProgressWhenGameOver(progress.getChildId(), progress.getGrade(), GameIdType.Location_Span.toString(), null);
		}catch(Exception e) {
			System.out.println("========================================== POWER PROGRAM ERROR ==========================================");
			e.printStackTrace(System.out);
			return null;
