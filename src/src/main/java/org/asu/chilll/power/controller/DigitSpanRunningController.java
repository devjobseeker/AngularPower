...
import org.asu.chilll.power.service.DigitSpanRunningService;
...
@Controller
public class DigitSpanRunningController {
	@Autowired
	private DigitSpanRunningService gameService;
	...
	@RequestMapping(value = "/api/digitspanrunning/syncdata", method = RequestMethod.POST)
	public @ResponseBody SyncDataResult initSyncDataRecord(@RequestBody String formData) {
		try {
			...
			return redcapService.initSyncDataRecords(student.getStudentId(), student.getGrade(), GameIdType.Digit_Span_Running.toString());
		}catch(Exception e) {
	...
	@RequestMapping(value = "/api/digitspanrunning/progress", method = RequestMethod.GET)
	public @ResponseBody GameProgressDataView fetchGameProgress(@RequestParam("studentId") String studentId, @RequestParam("grade") String grade) {
		try {
			//String childId = session.getAttribute(SessionAttributeName.Child_Id.toString()).toString();
			GameProgressDataView progress = progressService.fetchGameProgress(studentId, grade, GameIdType.Digit_Span_Running.toString());
			return progress;
		}catch(Exception e) {
			System.out.println("========================================== POWER PROGRAM ERROR ==========================================");
			e.printStackTrace(System.out);
	...
	@RequestMapping(value = "/api/digitspanrunning/progress", method = RequestMethod.POST)
	public @ResponseBody String updateGameProgress(@RequestBody String formData) {
		try {
			...
			progress.setGameId(GameIdType.Digit_Span_Running.toString());
	...
	@RequestMapping(value = "/api/digitspanrunning", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
	...
	@RequestMapping(value = "/api/digitspanrunning/audio", method = RequestMethod.POST)
	public @ResponseBody String sendAudioData(MultipartHttpServletRequest request, @RequestParam("childId") String childId,
		...
		try {
			...
			while(itr.hasNext()) {
				...
				dv.setGrade(grade);
				dv.setGameId(GameIdType.Digit_Span_Running.toString());
				dv.setFileLength(file.getSize());
				dv.setFileContent(file.getBytes());
				dv.setFileType(file.getContentType());
				dv.setFileName(GameIdType.Digit_Span_Running.toString() + "_" + file.getOriginalFilename());
				dv.setFolderName(FileFolderName.Digit_Span_Running.toString());
				dvs.add(dv);
	...
	@RequestMapping(value = "/api/digitspanrunning/gameover", method = RequestMethod.POST)
	public @ResponseBody GameProgressDataView gameOver(@RequestBody String formData) {
		try {
			GameProgressDataView progress = new Gson().fromJson(formData, GameProgressDataView.class);
			return progressService.updateGameProgressWhenGameOver(progress.getChildId(), progress.getGrade(), GameIdType.Digit_Span_Running.toString(), null);
		}catch(Exception e) {
			System.out.println("========================================== POWER PROGRAM ERROR ==========================================");
			e.printStackTrace(System.out);
			return null;
