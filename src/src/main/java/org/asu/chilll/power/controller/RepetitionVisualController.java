...
import org.asu.chilll.power.service.RepetitionVisualService;
...
@Controller
public class RepetitionVisualController {
	@Autowired
	private GameProgressService progressService;
	
	@Autowired
	private RepetitionVisualService gameService;
	
	@Autowired
	private REDCapSyncDataService redcapService;
	
	@RequestMapping(value = "/api/repetitionvisual/syncdata", method = RequestMethod.POST)
	public @ResponseBody SyncDataResult initSyncDataRecord(@RequestBody String formData) {
		try {
			...
			return redcapService.initSyncDataRecords(student.getStudentId(), student.getGrade(), GameIdType.Repetition_Detection_Visual.toString());
		}catch(Exception e) {
	...
	@RequestMapping(value = "/api/repetitionvisual/progress", method = RequestMethod.GET)
	public @ResponseBody GameProgressDataView fetchGameProgress(@RequestParam("studentId") String studentId, @RequestParam("grade") String grade) {
		try {
			GameProgressDataView progress = progressService.fetchGameProgress(studentId, grade, GameIdType.Repetition_Detection_Visual.toString());
			return progress;
		}catch(Exception e) {
			System.out.println("========================================== POWER PROGRAM ERROR ==========================================");
			e.printStackTrace(System.out);
	...
	@RequestMapping(value = "/api/repetitionvisual/progress", method = RequestMethod.POST)
	public @ResponseBody String updateGameProgress(@RequestBody String formData) {
		try {
			...
			progress.setGameId(GameIdType.Repetition_Detection_Visual.toString());
	...
	@RequestMapping(value = "/api/repetitionvisual", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
	...
	@RequestMapping(value = "/api/repetitionvisual/gameover", method = RequestMethod.POST)
	public @ResponseBody GameProgressDataView gameOver(@RequestBody String formData) {
		try {
			GameProgressDataView progress = new Gson().fromJson(formData, GameProgressDataView.class);
			return progressService.updateGameProgressWhenGameOver(progress.getChildId(), progress.getGrade(), GameIdType.Repetition_Detection_Visual.toString(), null);
		}catch(Exception e) {
			System.out.println("========================================== POWER PROGRAM ERROR ==========================================");
			e.printStackTrace(System.out);
			return null;
