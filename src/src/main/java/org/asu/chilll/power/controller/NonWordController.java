...
import org.asu.chilll.power.service.NonWordService;
...
@Controller
public class NonWordController {
	@Autowired
	...
	@Autowired
	private NonWordService gameService;
	
	@Autowired
	private REDCapSyncDataService redcapService;
	
	@RequestMapping(value = "/api/nonword/syncdata", method = RequestMethod.POST)
	public @ResponseBody SyncDataResult initSyncDataRecord(@RequestBody String formData) {
		try {
			...
			return redcapService.initSyncDataRecords(student.getStudentId(), student.getGrade(), GameIdType.Nonword_Repetition.toString());
		}catch(Exception e) {
	...
	@RequestMapping(value = "/api/nonword/progress", method = RequestMethod.GET)
	public @ResponseBody GameProgressDataView fetchGameProgress(@RequestParam("studentId") String studentId, @RequestParam("grade") String grade) {
		try {
			GameProgressDataView progress = progressService.fetchGameProgress(studentId, grade, GameIdType.Nonword_Repetition.toString());
			return progress;
		}catch(Exception e) {
			System.out.println("========================================== POWER PROGRAM ERROR ==========================================");
			e.printStackTrace(System.out);
	...
	@RequestMapping(value = "/api/nonword/progress", method = RequestMethod.POST)
	public @ResponseBody String updateGameProgress(@RequestBody String formData) {
		try {
			...
			progress.setGameId(GameIdType.Nonword_Repetition.toString());
	...
	@RequestMapping(value = "/api/nonword", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public @ResponseBody GameProgressDataView sendKeyBoardData(@RequestBody String formData) {
		try {
			NonWordUserData userData = new Gson().fromJson(formData, NonWordUserData.class);
	...
	@RequestMapping(value = "/api/nonword/audio", method = RequestMethod.POST)
	public @ResponseBody DocumentDataView sendAudioData(MultipartHttpServletRequest request, @RequestParam("childId") String childId,
		...
		try {
			...
			while(itr.hasNext()) {
				...
				dv.setGrade(grade);
				dv.setGameId(GameIdType.Nonword_Repetition.toString());
				dv.setFileLength(file.getSize());
				dv.setFileContent(file.getBytes());
				dv.setFileType(file.getContentType());
				dv.setFileName(GameIdType.Nonword_Repetition.toString() + "_" + file.getOriginalFilename());
				dv.setFolderName(FileFolderName.Nonword_Repetition.toString());
				dvs.add(dv);
	...
	@RequestMapping(value = "/api/nonword/gameover", method = RequestMethod.POST)
	public @ResponseBody GameProgressDataView gameOver(@RequestBody String formData) {
		try {
			GameProgressDataView progress = new Gson().fromJson(formData, GameProgressDataView.class);
			return progressService.updateGameProgressWhenGameOver(progress.getChildId(), progress.getGrade(), GameIdType.Nonword_Repetition.toString(), null);
		}catch(Exception e) {
			System.out.println("========================================== POWER PROGRAM ERROR ==========================================");
			e.printStackTrace(System.out);
			return null;
